...
| `_job-helpers.tpl` | One implementation of the pod spec, image resolution and the Job spec fields table (`jobSpecVerbatimFields`) for **every** hook and cronjob, both scopes — root-level callers simply pass no `deploy` |
