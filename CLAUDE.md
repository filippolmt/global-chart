...
| `_image-helpers.tpl` | `imageString`, `imagePullPolicy`. The registry test in `imageString` is mirrored by the image `pattern`s in `values.schema.json`: change one, change the other |
