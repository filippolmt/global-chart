...
| deployments | object | `{}` | Additional named deployments that inherit from the base `deployment` configuration |
...
Use the `deployments` map to spin up additional workloads derived from the base `deployment` block. Each key becomes a stable suffix
that is appended to the generated names so Services, Deployments, ConfigMaps, and other resources remain unique.
...
The example above renders two distinct deployment/service pairs: the base release name plus `-api` for the additional backend.
