...
{{- range $name, $deploy := .Values.deployments -}}
  ...
  {{- if eq (include "global-chart.deploymentEnabled" $deploy) "true" -}}
    ...
    {{- if ternary $deploySA.create true (hasKey $deploySA "create") -}}
      {{- $depSAName := include "global-chart.deploymentServiceAccountName" (dict "root" $root "deploymentName" $name "deployment" $deploy) -}}
      {{- include "global-chart.registerSAName" (dict "names" $saNames "sa" (dict "create" true "name" $depSAName) "owner" (printf "deployment '%s'" $name)) -}}
    {{- end -}}
...
copy of ADR 0002 deliberately shares the real SA's name, but it is a hook resource
...
Accepts: names (the accumulator dict, mutated in place), sa (a jobServiceAccount
result: .create + .name), owner (human-readable source, used in the message).
*/}}
...
{{- if and $sa.create $sa.name -}}
  {{- if hasKey $names $sa.name -}}
    {{- fail (printf "Name collision: ServiceAccount '%s' created for %s conflicts with the one created for %s. Give one of them an explicit serviceAccount.name, or set serviceAccount.create: false to bind the existing one." $sa.name .owner (index $names $sa.name)) -}}
  {{- end -}}
