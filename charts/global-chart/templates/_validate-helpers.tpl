...
mounted config file ConfigMaps, hook-prerequisite ConfigMaps, and
hook-prerequisite Secrets.
...
{{- range $name, $deploy := .Values.deployments -}}
  ...
  {{- if eq (include "global-chart.deploymentEnabled" $deploy) "true" -}}
    ...
    {{- range $i, $f := (default (list) $mcf.files) -}}
      {{- $mcName := include "global-chart.mountedConfigMapName" (dict "deploymentFullname" $depFullname "fileName" $f.name) -}}
      {{- if hasKey $cmNames $mcName -}}
        {{- fail (printf "Name collision: ConfigMap '%s' generated by mountedConfigFiles.files[%d] ('%s') in deployment '%s' conflicts with %s. Give one of the two entries a different 'name'." $mcName $i $f.name $name (index $cmNames $mcName)) -}}
      {{- end -}}
      {{- $_ := set $cmNames $mcName (printf "mountedConfigFiles.files[%d] ('%s') in deployment '%s'" $i $f.name $name) -}}
    {{- end -}}
    {{- range $bi, $b := (default (list) $mcf.bundles) -}}
      {{- range $fi, $f := (default (list) $b.files) -}}
        {{- $mcName := include "global-chart.mountedConfigMapName" (dict "deploymentFullname" $depFullname "fileName" $f.name) -}}
        {{- if hasKey $cmNames $mcName -}}
          {{- fail (printf "Name collision: ConfigMap '%s' generated by mountedConfigFiles.bundles[%d].files[%d] ('%s') in deployment '%s' conflicts with %s. Give one of the two entries a different 'name'." $mcName $bi $fi $f.name $name (index $cmNames $mcName)) -}}
        {{- end -}}
        {{- $_ := set $cmNames $mcName (printf "mountedConfigFiles.bundles[%d].files[%d] ('%s') in deployment '%s'" $bi $fi $f.name $name) -}}
      {{- end -}}
    {{- end -}}
