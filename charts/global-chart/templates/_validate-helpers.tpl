...
{{- range $i, $role := (default (dict) .Values.rbacs).roles -}}
  {{- if hasKey $rbacConsumers $role.name -}}
    ...
    {{- $sa := include "global-chart.rbacServiceAccount" $role | fromJson -}}
    {{- if $sa.create -}}
      {{- include "global-chart.registerSAName" (dict "names" $saNames "sa" (dict "create" true "name" (include "global-chart.rbacCopyServiceAccountName" (dict "root" $root "name" $sa.name))) "owner" $owner "hint" $copyHint) -}}
    {{- end -}}
...
and never reaches this validator. The rbacs.roles copy of ADR 0010 has its own
name, and does.
