...
lint-chart:
	helm lint $(STRICT) -f tests/values.01.yaml ./${GLOBAL_CHART_NAME}
	helm lint $(STRICT) -f tests/values.02.yaml ./${GLOBAL_CHART_NAME}
	helm lint $(STRICT) -f tests/values.03.yaml ./${GLOBAL_CHART_NAME}

generate-templates: lint-chart
	@rm -r generated-manifests || true
	@mkdir -p generated-manifests
	helm template test01-${GLOBAL_CHART_NAME} ./${GLOBAL_CHART_NAME} \
		-f tests/values.01.yaml \
		--namespace test \
		--output-dir generated-manifests/01 \
		--include-crds
	helm template test02-${GLOBAL_CHART_NAME} ./${GLOBAL_CHART_NAME} \
		-f tests/values.02.yaml \
		--namespace test \
		--output-dir generated-manifests/02 \
		--include-crds
	helm template test03-${GLOBAL_CHART_NAME} ./${GLOBAL_CHART_NAME} \
...
helm-install-${GLOBAL_CHART_NAME}-01:
	helm upgrade --install test ./${GLOBAL_CHART_NAME} \
		-f tests/values.01.yaml \
		--namespace test01 \
		--create-namespace
